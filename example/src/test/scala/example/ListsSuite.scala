...
class ListsSuite extends FunSuite {
  ...
  test("sum with 2 identical negative numbers") {
    assert(sum(List(-1,-1,0)) === -2)
  }
  ...
  test("max of a 3 zeros") {
    assert(max(List(0, 0, 0)) === 0)
  }

  test("max of 2 identical negative numbers") {
    assert(max(List(-1, 1, -1)) === 1)
  }

  test("max throws an exception if the input list is empty") {
